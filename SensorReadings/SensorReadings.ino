...
void displayWiFiStatus() {
    ...
    case WL_IDLE_STATUS:
    default:
		s = String("Connecting to WiFi ") + String(spinner());
		Heltec.display->drawString(0, 0, s);
        break;
	}
}
...
void messageReceived(String &topic, String &payload) {
  lastMsgTopic = topic; lastMsgPayload = payload;
  Serial.println("Message received: " + topic + " - " + payload);
}

// Might eventually be useful if we switch to HTTP
void makeHttpRequest(String host, String url) {
...
void loop(){
  ...
  if (timeSetup && !cloudSetup && connected) {
    ...
    device = new CloudIoTCoreDevice(
        "hombrew-iot", "europe-west1", "homebrew", deviceId, ESP32_PRIVATE_KEY);
